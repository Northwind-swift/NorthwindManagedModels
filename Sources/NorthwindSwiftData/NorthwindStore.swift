...
public enum NorthwindStore {
   ...
   */
  public static var schema = Schema.model(for: NorthwindSchema.self)
  ...
  public static func readOnlyModelContainer() throws -> ModelContainer {
    let config = ModelConfiguration(
      "NorthwindDataReadOnly",
      schema: schema, url: url, allowsSave: false,
      cloudKitDatabase: .none
    )
    assert(config.url == url, "Input URL is not the same as the output \(url)")
                       ...
                       throws -> URL
  {
    ...
    let walURL    = folderURL.appendingPathComponent(filename + "-wal")
    let shmURL    = folderURL.appendingPathComponent(filename + "-shm")
